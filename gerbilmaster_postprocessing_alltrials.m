...
for isubject = 1:size(curr_subject_ID,1)
    ...
    for itrial = 1:size(this_EEG.data,3)% for each trial (should be 144)
        ...
        for ionset = 1:length(this_trial_target_onsets) % for each target word onset...
            ...
            if ~isempty(ERP_info_target)
                ...
                ERP_info_target.Word = [ERP_info_target.Word; all_target_words(itrial).words(ionset)];
                ...
                ERP_info_target(1).Word = all_target_words(itrial).words(ionset);
    ...
    all_data_button_press(isubject,:,:,:) = data_by_button_press_baselined;
    all_data_target(isubject,:,:,:) = data_by_target_onset_baselined;
    all_data_masker(isubject,:,:,:) = data_by_masker_onset_baselined;
    ...
    subplot(size(curr_subject_ID,1),3,subplot_counter)
    hold on
    ...
    figure(3);
    subplot(size(curr_subject_ID,1),1,isubject)
    plot(single_onset_time,squeeze(mean(all_data_button_press(isubject,1,:,:),[1,3])))
    if isubject == size(curr_subject_ID,1)
        xlabel('Time (ms)','FontSize',18)
    elseif isubject == round(size(curr_subject_ID,1)/2)
        ylabel('Voltage (mV)','FontSize',18)
