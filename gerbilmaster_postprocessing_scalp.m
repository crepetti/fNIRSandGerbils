...
if whos_using == 'Ben'
    ...
    load 'C:\Users\ema36\OneDrive\Documents\fNIRSandGerbils\chan_locs_cart.txt'
end
curr_subject_ID = char('7006','7007','7008','7009');
...
%     end
    ...
    %% Movies Per Condition Response
    % 2-D movie
   
    figure; 
    %color words, scrambled
    all_scrambled_target_color_onset = cat(1, all_scrambled_st_by_target_white_onset, all_scrambled_st_by_target_blue_onset, all_scrambled_st_by_target_red_onset, all_scrambled_st_by_target_green_onset, all_scrambled_dt_by_target_white_onset, all_scrambled_dt_by_target_blue_onset, all_scrambled_dt_by_target_red_onset, all_scrambled_dt_by_target_green_onset);
    [Movie,Colormap] = eegmovie(squeeze(mean(all_scrambled_target_color_onset, 1)), srate, EEG.chanlocs, 'title', 'Color Words Averaged Across Talker, Scrambled');
    seemovie(Movie,1,Colormap);
    %save
    vidObj = VideoWriter('scrambled_color_onset_7006thru7009.mp4', 'Uncompressed AVI');
    open(vidObj);
    ...
    %object words, scrambled
    figure;
    all_scrambled_target_object_onset = cat(1, all_scrambled_st_by_target_object_onset, all_scrambled_dt_by_target_blue_onset);
    [Movie,Colormap] = eegmovie(squeeze(mean(all_scrambled_target_object_onset, 1)), srate, EEG.chanlocs, 'title', 'Object Words Averaged Across Talker, Scrambled');
    seemovie(Movie,1,Colormap);
    %save
    vidObj = VideoWriter('scrambled_object_onset_7006thru7009.mp4', 'Uncompressed AVI');
    open(vidObj);
    writeVideo(vidObj, Movie);
    close(vidObj);

    %masker words, scrambled
    figure;
    all_scrambled_target_masker_onset = cat(1, all_scrambled_st_by_masker_onset, all_scrambled_dt_by_masker_onset);
    [Movie,Colormap] = eegmovie(squeeze(mean(all_scrambled_target_masker_onset, 1)), srate, EEG.chanlocs, 'title', 'Masker Words Averaged Across Talker, Scrambled');
    seemovie(Movie,1,Colormap);
    %save
    vidObj = VideoWriter('scrambled_masker_onset_7006thru7009.mp4', 'Uncompressed AVI');
    open(vidObj);
    writeVideo(vidObj, Movie);
    close(vidObj);

    %color words, unscrambled
    figure;
    all_unscrambled_target_color_onset = cat(1, all_unscrambled_st_by_target_white_onset, all_unscrambled_st_by_target_blue_onset, all_unscrambled_st_by_target_red_onset, all_unscrambled_st_by_target_green_onset, all_unscrambled_dt_by_target_white_onset, all_unscrambled_dt_by_target_blue_onset, all_unscrambled_dt_by_target_red_onset, all_unscrambled_dt_by_target_green_onset);
    [Movie,Colormap] = eegmovie(squeeze(mean(all_unscrambled_target_color_onset, 1)), srate, EEG.chanlocs, 'title', 'Color Words Averaged Across Talker, Unscrambled');
    seemovie(Movie,1,Colormap);
    %save
    vidObj = VideoWriter('unscrambled_color_onset_7006thru7009.mp4', 'Uncompressed AVI');
    open(vidObj);
    writeVideo(vidObj, Movie);
    close(vidObj);
    %object words, unscrambled
    figure;
    all_unscrambled_target_object_onset = cat(1, all_unscrambled_st_by_target_object_onset, all_unscrambled_dt_by_target_object_onset);
    [Movie,Colormap] = eegmovie(squeeze(mean(all_unscrambled_target_object_onset, 1)), srate, EEG.chanlocs, 'title', 'Object Words Averaged Across Talker, Unscrambled');
    seemovie(Movie,1,Colormap);
    %save
    vidObj = VideoWriter('unscrambled_object_onset_7006thru7009.mp4', 'Uncompressed AVI');
    open(vidObj);
    writeVideo(vidObj, Movie);
    close(vidObj);

    %masker words, unscrambled
    figure;
    all_unscrambled_target_masker_onset = cat(1, all_unscrambled_st_by_masker_onset, all_unscrambled_dt_by_masker_onset);
    [Movie,Colormap] = eegmovie(squeeze(mean(all_unscrambled_target_masker_onset, 1)), srate, EEG.chanlocs, 'title', 'Masker Words Averaged Across Talker, Unscrambled');
    seemovie(Movie,1,Colormap);
    %save
    vidObj = VideoWriter('unscrambled_masker_onset_7006thru7009.mp4', 'Uncompressed AVI');
    open(vidObj);
    writeVideo(vidObj, Movie);
    close(vidObj);

    %color words, scrambled - unscrambled
    figure;
    all_scrambled_minus_unscrambled_target_color_onset = all_scrambled_target_color_onset-  all_unscrambled_target_color_onset;
    [Movie,Colormap] = eegmovie(squeeze(mean(all_scrambled_minus_unscrambled_target_color_onset, 1)), srate, EEG.chanlocs, 'title', 'Color Word Scrambled Minus Unscrambled');
    seemovie(Movie,1,Colormap);
    %save
    vidObj = VideoWriter('scrambled_minus_unscrambled_color_onset_7006thru7009.mp4', 'Uncompressed AVI');
    open(vidObj);
    writeVideo(vidObj, Movie);
    close(vidObj);

    % Across scrambled and unscrambled, target color minus target object
    figure;
    all_color = cat(1,all_scrambled_target_color_onset,all_unscrambled_target_color_onset);
    all_object = cat(1,all_scrambled_target_object_onset,all_unscrambled_target_object_onset);
    [Movie,Colormap] = eegmovie(squeeze(mean(all_color, 1)) - squeeze(mean(all_object,1)), srate, EEG.chanlocs, 'title', 'Color Word Minus Object Word (Target, both conditions)');
    seemovie(Movie,1,Colormap);
    %save
    vidObj = VideoWriter('color_minus_object_onset_7006thru7009.mp4', 'Uncompressed AVI');
    open(vidObj);
    writeVideo(vidObj, Movie);
    close(vidObj);
